...
Since we're looking at 1000 lambda values for each year, it is useful to look at plots for a range of r values. I've broken these into r-values 0-0.49 (declining population), 0.5-1.49, and 1.5-2.
```{r}
r_low<-(r1976_sim[names(r1976_sim)<0.5])
mean(as.numeric(names(r_low)))
df_low<-ldply(r_low, data.frame)

r_mid<-r1976_sim[names(r1976_sim)>=0.5 & names(r1976_sim)<1.5]
mean(as.numeric(names(r_mid)))
df_mid<-ldply(r_mid, data.frame)

r_high<-r1976_sim[names(r1976_sim)>=1.5]
mean(as.numeric(names(r_high)))
df_high<-ldply(r_high, data.frame)
```

```{r, echo=FALSE}
time20<-subset(df_low, time==20, select=c(.id, N, P))
plot(time20$.id, time20$N, main="r value versus population density at time 20", xlab="r value", ylab="individuals/ha", col=3, lty=1, ylim=c(0,75))
