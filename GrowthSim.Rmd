...
for(i in 1:length(r[[2]])){
  r1977_sim[[i]]<- ode(y=state, times = times, func = pred_prey, parms = c(r = r[[2]][i], alpha, beta = 0.002, delta = 0.01, K_prey=175, k_max, D))
}


r1978_sim<-setNames(vector("list", length(r[[3]])), as.character(r[[3]]))
for(i in 1:length(r[[3]])){
  r1978_sim[[i]]<- ode(y=state, times = times, func = pred_prey, parms = c(r = r[[3]][i], alpha, beta = 0.002, delta = 0.01, K_prey=175, k_max, D))
}
```
