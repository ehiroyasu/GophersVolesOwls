...
ggplot(NPeq) + 
  ...
  geom_hline(yintercept=0, linetype="dashed", color="red") +
  scale_x_continuous(expand= c(0,0)) + 
  labs(title="N'* as a function of predator density (D=0.5)", x="Predator density", y="N'*") #to add labels, use the following code
...
ggplot(NDeq) + 
  ...
  geom_hline(yintercept=0, linetype="dashed", color="red") +
  scale_x_continuous(expand= c(0,0)) + 
  labs(title="N'* as a function of D (P=0.5)", x="D", y="N'*")
```

```{r}

Dprime<-seq(-10, 10, by=0.1)

P_func<-function(Dprime){
  P<-data.frame((0.25*((Dprime-1)^2))+Dprime)
  P.df<-data.frame(P, Dprime)
  names(P.df)<-c("P", "Dprime")
...
ggplot(P.df)+
  geom_line(aes(Dprime, P)) +
  geom_abline(intercept=0, slope=1, linetype="dashed") + 
  labs(title="Predator density as a function of D", x="D", y="P") +
  scale_y_continuous(expand=c(0,0)) + geom_vline(xintercept=0) +
  geom_hline(yintercept=0) + coord_cartesian(xlim=c(-3,3), ylim=c(-3,3))+
  theme_minimal()
