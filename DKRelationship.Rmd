...
\[N_{eq}=\frac{(K_{prey}-D) \pm \sqrt{(K_{prey}-D)^2 + 4 \frac{K_{prey}}{r} (rD-k_{max} P)}}{2}\]
...
beta = 0.00054
delta=0.01
K_prey=c(1000, 750, 500)
k_max=655
#h=1/k_max
D=seq(0, 500, by=10)#c(50, 70, 100, 250, 500) #where D is specified, however can be calculated as 1/alpha*h
r=c(0.5, 1, 1.5, 2)
...
Using the quadratic formula, we can calculate the equilibrium densities of the prey population. N1 is the solution to $N_{eq}=\frac{(K_{prey}-D) + \sqrt{(K_{prey}-D)^2 + 4 \frac{K_{prey}}{r} (1-k_{max} P)}}{2}$. N2 is the solution to $N_{eq}=\frac{(K_{prey}-D) - \sqrt{(K_{prey}-D)^2 + 4 \frac{K_{prey}}{r} (1-k_{max} P)}}{2}$


```{r, echo=FALSE, fig.align="left"}
eq<-N_eq(P, r, K_prey, D, k_max)

P_split<-split(eq, P)
#plot_eq(P_split[[1]], P, i=1)

for(i in 1:length(P_split)){
  print(plot_eq(df=P_split[[i]], P=P, i=i))
}

##not quite
#ggplot(eq, aes(x = r, y = Eq, group=N, color=as.factor(P))) + geom_point(size = 0.5)

#Not quite...
#ggplot(P_split[[1]], aes(x=r, y=Eq, group=N, color=N)) + geom_point(size=0.5)

#this is closer to what I'm looking for
#ggplot(P_split[[1]], aes(x = D, y = Eq, group=N, color=as.factor(r))) + geom_point(size = 0.5)

#maybe this?
r0.5<-subset(P_split[[1]], r==0.5)
r0.5<-subset(r0.5, K_prey==1000)
ggplot(r0.5, aes(x=D, y=Eq, group=N, color=N)) + geom_point() + ggtitle("Bifurcation Plot of N* vs D, r=0.5, P=0.2, K=1000")
```

Plotting the relationship between N* and D
```{r, echo=FALSE}
for (j in 1:length(P_split)){
  foo<-bifplot(P_split, K_prey, i=j)
  ggsave(paste("P=",P[j], ".png"), foo, device="png", 
         path="D:/GopherProject/GophersVolesOwls/Figures/DKPlots")
}
