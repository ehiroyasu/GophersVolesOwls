---
title: "Relationship between D and N"
...
##Examining the relationship between D and P
Looking at the equilibrium under the root, we find
```{r}
Dprime<-D
Pprime_pos<-NULL
Pprime_neg<-NULL
...
for(i in 1:length(Dprime)){
  Pprime_pos[i]<-((-Dprime[i]^2)+(6*Dprime[i])-1)/4
  Pprime_neg[i]<-((Dprime[i]^2)-(6*Dprime[i])+1)/4
}

df<-cbind(Pprime_pos, Pprime_neg, Dprime)

plot(Pprime_neg~Dprime, data=df, ylim=c(-2,5))
abline(a=0, b=1)

plot(Pprime_pos~Dprime, data=df, ylim=c(-0.5, 0.5))
lines(Pprime_neg~Dprime)
```
